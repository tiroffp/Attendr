...
class ModelTest(TestCase):
    ...
    def test_default_text(self):
        attendee = Attendee()
        self.assertEqual(attendee.text, '')
    ...
    def test_attendee_is_related_to_roll(self):
        ...
        attendee.roll = roll
        attendee.save()
        self.assertIn(attendee, roll.item_set.all())

    def test_cannot_save_empty_roll_attendees(self):
        roll = Roll.objects.create()
        attendee = Attendee(roll=roll, text='')
    ...
    def test_dupliacte_attendees_are_invalid(self):
        roll = Roll.objects.create()
        Attendee.objects.create(roll=roll, text='bla')
        with self.assertRaises(ValidationError):
            attendee = Attendee(roll=roll, text='bla')
            attendee.full_clean()

    def test_CAN_save_attendee_to_different_create_roll(self):
        roll1 = Roll.objects.create()
        roll2 = Roll.objects.create()
        Attendee.objects.create(roll=roll1, text='bla')
        attendee = Attendee(roll=roll2, text='bla')
