...
class RollViewTest(TestCase):
    def test_displays_only_attendees_for_that_roll(self):
        ...
        Attendee.objects.create(text='attendeey 1', roll=correct_roll)
        Attendee.objects.create(text='attendeey 2', roll=correct_roll)
        other_roll = Roll.objects.create()
        Attendee.objects.create(text='other roll attendee 1', roll=other_roll)
        Attendee.objects.create(text='other roll attendee 2', roll=other_roll)
    ...
    def test_can_save_a_POST_request_to_an_existing_roll(self):
        ...
        self.client.post(
            ...
            data={'text': 'A new attendee for an existing roll'}
        )

        self.assertEqual(Attendee.objects.count(), 1)
        new_attendee = Attendee.objects.first()
        self.assertEqual(new_attendee.text, 'A new attendee for an existing roll')
...
class NewRollTest(TestCase):
    def test_saving_a_POST_request(self):
        self.client.post('/create_roll/new',
                         data={'text': 'A new roll attendee'})
        self.assertEqual(Attendee.objects.count(), 1)
        new_attendee = Attendee.objects.first()
        self.assertEqual(new_attendee.text, 'A new roll attendee')
    ...
    def test_for_invalid_input_renders_home_template(self):
        response = self.client.post('/create_roll/new', data={'text': ''})
        self.assertEqual(response.status_code, 200)
        self. assertTemplateUsed(response, 'home.html')
    ...
    def test_invalid_roll_attendees_arent_saved(self):
        self.client.post('create_roll/new', data={'text': ''})
